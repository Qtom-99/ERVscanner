...
while getopts "i:s:t:d:n:m:q:" opt; do
  case $opt in
    s) SAMPLE="$OPTARG" ;;
    i) INPUT_PATH="$OPTARG" ;;
    t) INPUT_TYPE="$OPTARG" ;;
...
python3 $DATA_PATH/script/extract_sample_pos.py ${line} $DATA_PATH/01table/allsample_01table_${threshold//./}up.tsv $DATA_PATH/allsample_merge/pos.bed $DATA_PATH/sampledata/${line}/clip/${line}_pos.bed
...
samtools view -@ $NCORE -q 30 -o "$DATA_PATH"/genotype/sampledata/${line}/${line}_muttype_q${QUALITY}.bam "$DATA_PATH"/genotype/sampledata/${line}/${line}_muttype.bam
samtools view -@ $NCORE -h $DATA_PATH/genotype/sampledata/${line}/${line}_muttype_q${QUALITY}.bam | awk '$6 !~ /[SH]/ || $1 ~ /^@/' | samtools view -b -o $DATA_PATH/genotype/sampledata/${line}/${line}_q${QUALITY}_no_clipping.bam -
bedtools bamtobed -i $DATA_PATH/genotype/sampledata/${line}/${line}_q${QUALITY}_no_clipping.bam > $DATA_PATH/genotype/sampledata/${line}/${line}_q${QUALITY}_no_clipping.bed
sort -V -k1,1 -k2,2 $DATA_PATH/genotype/sampledata/${line}/${line}_q${QUALITY}_no_clipping.bed > $DATA_PATH/genotype/sampledata/${line}/${line}_q${QUALITY}_no_clipping_sort.bed
cut -f 1,2,3 $DATA_PATH/genotype/sampledata/${line}/${line}_q${QUALITY}_no_clipping_sort.bed > $DATA_PATH/genotype/sampledata/${line}/${line}_q${QUALITY}_no_clipping_sort123.bed
...
bedtools intersect -a $DATA_PATH/sampledata/${line}/clip/${line}_breakpoint.bed -b $DATA_PATH/genotype/sampledata/${line}/${line}_q${QUALITY}_no_clipping_sort123.bed -f 1.0 -c > $DATA_PATH/genotype/sampledata/$line/${line}_n_of_covreads_noclip.txt
